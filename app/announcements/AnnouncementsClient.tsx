...
import { SkeletonCardGrid } from "@/app/_components/Skeleton";
...
interface AnnouncementsClientProps {
    ...
    initialOpportunities: Announcement[];
}
...
export default function AnnouncementsClient({
    ...
    const [announcements, setAnnouncements] = useState<Announcement[]>(initialAnnouncements.slice(0, ITEMS_PER_PAGE));
    const [allItems, setAllItems] = useState<Announcement[]>(activeTab === 'announcements' ? initialAnnouncements : initialOpportunities);
    const [loadingMore, setLoadingMore] = useState(false);
    const [page, setPage] = useState(1);
    const [hasMore, setHasMore] = useState(initialAnnouncements.length > ITEMS_PER_PAGE);
    const [totalPages, setTotalPages] = useState(Math.ceil(initialAnnouncements.length / ITEMS_PER_PAGE));
    const { language, t } = useLanguage();

    useEffect(() => {
        const items = activeTab === 'announcements' ? initialAnnouncements : initialOpportunities;
        setAllItems(items);
        setAnnouncements(items.slice(0, ITEMS_PER_PAGE));
        setPage(1);
        setHasMore(items.length > ITEMS_PER_PAGE);
        setTotalPages(Math.ceil(items.length / ITEMS_PER_PAGE));
    }, [activeTab, initialAnnouncements, initialOpportunities]);
