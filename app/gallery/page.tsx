          ...
          {announcements.map((a) => (
            <Link key={a.slug} href={`/gallery/${a.slug}`} className="bg-secondary-800/50 rounded-xl shadow p-6 flex flex-col hover:ring-2 hover:ring-primary-400 transition-all">
              ...
              <div className="flex items-center justify-between mb-2">
                <span
                  className={`px-3 py-1 rounded-full text-xs font-medium ring-1 ring-inset 
              ...
              <p className="text-gray-300 mb-4 line-clamp-3">{a.galleryDescription || a.description}</p>
            </Link>
          ))}
        </div>
      </div>
