...
export default function GalleryClient({ initialItems }: GalleryClientProps) {
    const [announcements, setAnnouncements] = useState<GalleryItem[]>(initialItems.slice(0, ITEMS_PER_PAGE));
    const [loadingMore, setLoadingMore] = useState(false);
    const [page, setPage] = useState(1);
    const [hasMore, setHasMore] = useState(initialItems.length > ITEMS_PER_PAGE);
    const [totalPages] = useState(Math.ceil(initialItems.length / ITEMS_PER_PAGE));
    ...
    const getTitle = (a: GalleryItem) => {
        if (language === 'en' && a.titleEn) return a.titleEn;
        return a.title;
    };

    const getDescription = (a: GalleryItem) => {
        if (language === 'en') {
    ...
    const getTypeLabel = (type: GalleryItem['type']) => {
        const typeMap = {
