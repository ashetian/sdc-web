...
import { SkeletonCalendar, SkeletonFullPage } from '../_components/Skeleton';
...
export default function EventsClient() {
    ...
    const getWeekEvents = (week: (Date | null)[]) => {
        ...
        const rawItems = [
            ...
            ...events.map(e => ({ ...e, isMarker: false, startDate: e.eventDate, endDate: e.eventEndDate || e.eventDate }))
        ].filter(item => {
                ...
                <div key={`week-${wIdx}`} className="grid grid-cols-7 relative bg-white border-b-2 border-black" style={{ minHeight: `${cellHeight}px` }}>
                    ...
                    <div className="absolute inset-0 top-9 w-full grid grid-cols-7 pointer-events-none px-1">
                        {processedItems.map((item: any, idx: number) => {
                            ...
                            const eventEndDate = new Date(item.data.endDate);
                            eventEndDate.setHours(23, 59, 59, 999);
                            const isPastEvent = !isMarker && eventEndDate < today;

                            const bgColor = isMarker ? (config?.bg || 'bg-gray-200') : (isPastEvent ? 'bg-gray-300' : 'bg-neo-purple');
                            const textColor = isMarker ? (config?.text || 'text-black') : (isPastEvent ? 'text-gray-600' : 'text-white');
                            const borderColor = isPastEvent ? 'border-gray-400' : 'border-black';
